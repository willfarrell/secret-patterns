#!/usr/bin/env sh
git secrets --add '("|''|`)?(AWS|aws|Aws)?_?(ACCESS|access|Access)_?(KEY|key|Key)?_?(ID|id|Id)?("|''|`)?\s*(:|=>|=)\s*("|''|`)?(A3T[A-Z0-9]|AKIA|AGPA|AIDA|AROA|AIPA|ANPA|ANVA|ASIA)[A-Z0-9]{16}("|''|`)?'
...
git secrets --add '("|''|`)?(AWS|aws|Aws)?_?(SECRET|secret|Secret)_?(ACCESS|access|Access)?_?(KEY|key|Key)?_?(ID|id|Id)?("|''|`)?\s*(:|=>|=)\s*("|''|`)?[A-Za-z0-9/+=]{40}("|''|`)?'
git secrets --add '("|''|`)?(AWS|aws|Aws)?_?(SESSION|session|Session)?_?(TOKEN|token|Token)?("|''|`)?\s*(:|=>|=)\s*("|''|`)?[A-Za-z0-9/+=]{16,4096}("|''|`)?'
git secrets --add '[f|F][a|A][c|C][e|E][b|B][o|O][o|O][k|K].*("|''|`)[0-9a-f]{32}("|''|`)'
git secrets --add '[a|A][p|P][i|I][_]?[k|K][e|E][y|Y].*("|''|`)[0-9a-zA-Z]{32,64}("|''|`)'
git secrets --add '[p|P][a|A][s|S][s|S][w|W]?[o|O][r|R]?[d|D]?.*("|''|`).{8,64}("|''|`)'
git secrets --add '[s|S][e|E][c|C][r|R][e|E][t|T].*("|''|`)[0-9a-zA-Z]{32,64}("|''|`)'
git secrets --add '[t|T][o|O][k|K][e|E][n|N].*("|''|`).{8,64}("|''|`)'
git secrets --add '[g|G][i|I][t|T][h|H][u|U][b|B].*("|''|`)[0-9a-zA-Z]{35,40}("|''|`)'
git secrets --add '("|''|`)type("|''|`)\s*(:|=>|=)\s*("|''|`)service_account("|''|`),?'
git secrets --add '(("|''|`)client_secret("|''|`)\s*(:|=>|=)\s*("|''|`)[a-zA-Z0-9-_]{24}("|''|`))'
...
git secrets --add 'SK[a-z0-9]{32}'
git secrets --add '[t|T][w|W][i|I][t|T][t|T][e|E][r|R].*("|''|`)[0-9a-zA-Z]{35,44}("|''|`)'
git secrets --add '[a-zA-Z]{3,10}://[^/\s:@]{3,20}:[^/\s:@]{3,20}@.{1,100}["''\s]'
git secrets --add -a 'AKIAIOSFODNN7EXAMPLE'
git secrets --add -a 'wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY'
